...
Aus \autoref{sec:usability} geht hervor, dass Elemente entsprechend ihrer Funktion gefärbt werden, nicht aus rein dekorativen Gründen oder zur thematischen Strukturierung. Aus diesen Gründen wird zwischen die Abbildung $CGs \to P$ eine Abstraktionsschicht mit der Bezeichnung \textbf{Farbfunktionen} $F$ geschoben, welche beschreibt, welche Funktion eine Color Group in einer Oberfläche erfüllt. In Anlehnung an \citep{google,  smashing} wird exemplarisch $S = \{\text{Primärfarbe}, \text{Sekundärfarbe}, \text{Interaktionsfarbe},\\ \text{Akzentfarbe}, \text{Textfarbe}, \text{Text-Hintergrund}\}$ definiert. Die einzelnen Farbunktionen lauten wie folgt:
\begin{itemize}
	...
	\item \textbf{Akzentfarbe:} Signalisiert Elemente mit hoher Priorität in der visuellen Hierarchie. Dies ist erreichbar durch einen verhältnismäßg seltenen (Quantitätskontrast, \citep{webdesign}) Einsatz einer bunten \citep{visual-hierarchy} Farbe.
\end{itemize}

TODO Kontraste in den Anhang.

Abhängig von den Ansprüchen der Webseite sind weitere Funktionsgruppen möglich, z.B. wenn zwischen verschiedenen Formen der Interaktion differenziert werden soll (wischen und drücken). Die hier genannten Farbfunktionen stellen lediglich einen Konsens der Literatur dar.
