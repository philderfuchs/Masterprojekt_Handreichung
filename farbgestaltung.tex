...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.7\textwidth]{img/text_color.png}
...
mit $L_c \in [1, ... , 22]$, wobei $L_c(rgb(255, 255, 255), rgb(0, 0, 0)) = 22$ und $c_1 = c_2 \implies L_c(c_1, c_2) = 1$ gilt. Die Berechnungsvorschrift der normalisierten relativen Luminanz $L()$ einer Farbe ist \citep{wcag-rel-luminance} zu entnehmen.
...
\begin{equation}
  	AA: L_c(c_1, c_2) \geq
	\begin{cases}
	3.0, \; \text{Schriftgr.} \geq 18\text{pt} \lor \geq 14pt \land \text{fett}\\
		4.5,  \;  \text{sonst}
	\end{cases}
\end{equation}
...
\begin{equation}
  	AAA: L_c(c_1, c_2) \geq
	\begin{cases}
		4.5, \; \text{Schriftgr.} \geq 18\text{pt} \; \lor \geq 14pt \land \text{fett}\\
		7.0,  \;  \text{sonst}
	\end{cases}
\end{equation}
