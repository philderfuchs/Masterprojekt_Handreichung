...
\autoref{fig:h_segmentation} lässt außerdem erkennen, dass die resultierende Farbpalette zwar die Farbdetails der Bildvorlage berücksichtigt, umgekehrt jedoch nicht sofort jede Farbe der Palette intuitiv einer Bildkomponente zugeordnet werden kann. Zur Reduzierung der gebildeten Abstufungen wird ein Weichzeichnungsfilter (Mittelwert) \citep{image-based-schemes} sowie Kompressor (Wurzelfunktion) auf das Histogramm angewandt. \autoref{fig:grey_cylinder} zeigt das Resultat nach Filterung des Grau-Zylinders, Weichzeichnung und anschließender Kompression des Histogramms. Als Nebeneffekt wird auf diese Weise der Suchraum für die Ermittlung des Farbschemas reduziert, wodurch Laufzeiten verbessert werden.

\begin{figure}[]
\centering
\includegraphics[width=0.95\textwidth]{img/grey_cylinder.png}
\caption{(a) Hierarchische Farbpalette ohne Vorverarbeitung des Histogramms. (b) Hierarchische Farbpalette mit Filterung der Grau-Zylinders ($S < \theta = 0.1$), Weichzeichnung und anschließender Kompression des Histogramms. Die Hue-Group mit Farben geringer Sättigung entfällt. Es werden weniger Sättigungs- und Helligkeitsstufen gebildet, die sich intuitiver den Farben der Bildvorlage zuordnen lassen.}
\label{fig:grey_cylinder}
\end{figure}

\citet{image-based-schemes} argumentiert, dass zusammenhängende Bereiche mit gleichem Farbton und hoher Sättigung eine besondere Relevanz für die Farbwahrnehmung von Bildern besitzen. Darum wird eine Gewichtung der Samples im Histogramm vorgeschlagen. Für den Wert $h(c)$ einer Farbe $c$ im Histogramm wird folgende Formel verwendet:
...
Dementsprechend erhalten Samples mit hohem Sättigungswert sowie mit ähnlichen Samples in der 8ter-Umgebung eine höhere Gewichtung.
